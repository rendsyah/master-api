...
export class UserService {
  ...
  async user(user: IUser): Promise<UserResponse> {
    const getUser: UserResponse = await this.model('user')
  ...
  async getUserList(dto: UserListDto): Promise<UserListResponse> {
    ...
    const baseQuery = this.model('user')
      .innerJoin('user_access AS access', 'access.id', 'user.access_id')
      .select(
        'user.id AS id',
        'user.fullname AS fullname',
    ...
    const [items, countResult] = await Promise.all([
      baseQuery.clone().orderBy(orderBy, sort).offset(Number(skip)).limit(Number(limit)),
      baseQuery.clone().clearSelect().clearOrder().count().first(),
    ]);
  ...
  async createAccess(dto: CreateAccessDto, user: IUser): Promise<CreateAccessResponse> {
    ...
    await this.model.transaction(async (trx) => {
      const access = await trx('user_access')
        .insert({
          name: formatName,
      ...
      await trx('user_access_detail').insert(mapDetail);
    });
