  ...
  constructor(private readonly httpAdapterHost: HttpAdapterHost) {}
    ...
    if (exception instanceof HttpException) {
      ...
      if (typeof response === 'object' && response !== null) {
        const { message: responseMessage, errors: responseErrors } = response as {
          message: string;
        ...
        fallbackResponse.message = String(response);
      }
    }

    httpAdapter.reply(ctx.getResponse(), fallbackResponse, statusCode);
  }
}
