...
export class UtilsService {
  ...
  public validateString(request: string, type: Utils.IValidateString): string {
    if (!request) return '';
  ...
  public validateReplacePhone(request: string, type: Utils.IValidateReplacePhone): string {
    if (!request) return '';
  ...
  public validateRandomChar(request: number, type: Utils.IValidateRandomChar): string {
    if (!request) return '';
  ...
  public validateFile(request: Express.Multer.File, config: Utils.IFile): string {
  ...
  public validateBase64File(request: string, config: Utils.IFile): string {
  ...
  public pagination(request: Utils.IPagination): Utils.IPagination {
    const getPage = request.page ? +request.page : 1;
    const getLimit = request.limit ? +request.limit : 10;
    const getIsActive = request.isActive != undefined ? +request.isActive : undefined;
    const getOrderBy = request.orderBy;
    const getSort = request.sort ? (request.sort as Utils.IPagination['sort']) : 'DESC';
  ...
  public paginationInfiniteResponse<T>(
    request: Utils.IPaginationResponse<T>,
  ): Utils.IPaginationResponse<T> {
